...
The Zone choice is used for duct simple model to calculate duct losses wihtout using the Airflow Network model.
...
\item
 Supply Branch duct: The connection of SupplyBranch is between the AirLoopHVAC:ZoneSplitter and the inlet node of a zone terminal. Each outlet of the AirLoopHVAC:ZoneSplitter has a single branch duct. The number of Supply Branch ducts are the same as the number of outlets of the AirLoopHVAC:ZoneSplitter for each AirLoopHVAC.
\item
 Return Branch duct: The connection of ReturnBranch is between the zone outlet node and the AirLoopHVAC:ZoneMixer.  Each outlet of the AirLoopHVAC:ZoneMixer has a single branch duct.The number of Return Branch ducts are the same as the number of inlets of the AirLoopHVAC:ZoneMixer for each AirLoopHVAC
\item
...
\begin{itemize}
\item
  Supply Trunk leak: The supply trunk duct leak occurs at the end of supply trunk duct. The leak is originated at the AirLoopHVAC:ZoneSplitter and terminated at a zone or outdoors.
\item
  Supply Branch leak: The supply branch duct leak occurs at the end of supply branch duct. The leak is originated at the zone terminal inlet node and terminated at a zone or outdoors.
\item
  Return Branch leak: The return branch duct leak occurs at the beginning of return branch duct. The leak is originated at a zone or outdoors and terminated at the zone outlet node.
\item
  Return Trunk leak: The return trunk duct leak occurs at the beginning of return trunk duct. The leak is originated at a zone or outdoors and terminated at the AirLoopHVAC:ZoneMixer.

\end{itemize}


Model assumptions are provided below:
...
\item
The Duct:Loss:Conduction objects for a given duct linkage should reference the same AirflowNetwork:Distribution:Component:Duct object.
\item
The Duct:Loss:Leakage objects for a given duct linkage should reference the same AirflowNetwork:Distribution:Component:LeakageRatio object.
\end{itemize}

\subsection{Duct:Loss:Conduction}\label{ductlossconduction}

This object allows users to input ducts with conduction loss only for simplified duct model.
...
\subsubsection{Field: AirLoopHAVC Name}\label{duct-conduction-airloophavcname}
...
The objects required for a complete set of inputs to simulate duct conduction losses are listed below
...
This object allows users to input duct leaks only for simplified duct model. 
...
The objects required for a complete set of inputs to simulate duct leakage losses are listed below
...
This object allows users to input makeup air caused by a sum of supply leaks greater than a sum of return leaks only for simplified duct model. 
...
The objects required for a complete set of inputs to simulate duct makeup air losses are listed below
...
\paragraph{Duct Loss Node Humidity Ratio {[}kgWater/kgDryAir{]}}\label{duct-loss-node-humidity-ratio-kgwaterkgdryair}
...
This is the duct loss sensible rate output in W for the simple duct model for each duct loss object.

\paragraph{Duct Loss Latent Loss Rate {[}kgWater/kgDryAir{]}}\label{duct-loss-latent-rate-kgwaterkgdryair}

This is the duct loas latent rate output in kgWater/kgDryAir for the simple duct model for each duct loss object.
