...
\paragraph{Availability Schedule Name Heating} A schedule that defines the
...
\paragraph{Field: Operating Mode Control Option for Multiple Unit} This
determines how the operation mode is assigned when the load is too large to be
met by multiple heat pump units. The default is SingleMode.
\begin{itemize}
...
pumps, they will be used to fulfill the remaining heating demand.
    \item HeatingPriority: similar to the previous cooling priority one, but
will prioritize to meet the heating demand first.
...
the control method is set to ScheduledModes Schedule values control operating
mode: 0=off, -1=cooling, 1=heating. Note that this only applies to the
simplified case where all units are in the same mode.
...
(capacity, COP, and SHR). The default value is 8$^\circ$C. The minimum value is
5$^\circ$C.
\paragraph{Field: Rated Air Flow Rate in Heating Mode} This field is the air
flow rate corresponding to rated heat pump performance (capacity, COP, and SHR).
...
(heating capacity, COP, and SHR). The default value is 40$^\circ$C. The minimum
value is 25$^\circ$C.
...
inlet air dry-bulb temperature corresponding to rated heat pump performance
(capacity, COP, and SHR). The default value is 30$^\circ$C. The minimum value is
5$^\circ$C.
...
the outlet water temperature corresponding to rated heat pump performance
(heating capacity, COP, and SHR). The default value is 8$^\circ$C. The minimum
value is 0$^\circ$C.
...
\paragraph{Field: Defrost Capacity Ratio Function of Temperature Curve Name}
...
\paragraph{Heat Pump Entering Water Temperature in Cooling Mode {[}C{]}} The
