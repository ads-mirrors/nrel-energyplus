...
Interior Lights \tabularnewline
Exterior Lights \tabularnewline
Interior Equipment \tabularnewline
Exterior Equipment \tabularnewline
Fans \tabularnewline
Pumps \tabularnewline
Heat Rejection \tabularnewline
Humidification \tabularnewline
Heat Recovery \tabularnewline
...
\paragraph{Field: Tabular Unit Conversion}\label{field-tabular-unit-conversion-json}

This field determines if any unit conversions will be performed for the values in the tabular reports when the Option field is set to TimeSeriesAndTabular.

This field is optional and, if not specified, defaults to UseOutputControlTableStyle which uses the same unit conversion that appears in the   \hyperref[outputcontroltablestyle]{OutputControl:Table:Style}
 object. Five different input options are available:

\begin{itemize}
\item 
UseOutputControlTableStyle -- use the same unit conversion as in   \hyperref[outputcontroltablestyle]{OutputControl:Table:Style}
...
\item 
InchPoundExceptElectricity -- similar to \textbf{InchPound} but using kWh (energy), and W or kW (rate) for electricity usages.
\end{itemize}

The JtoKWH, JtoMJ and JtoGJ unit conversion input option applies only to the Output:Table:Monthly reports and partially to the ABUPS report. For ABUPS, the JtoKWH option changes the report but the JtoMJ and JtoGJ options do not change the report since it is already in MJ/m2 and GJ. In addition, the JtoKWH option also changes results in the LEED Summary report changing GJ to kWh and MJW/m2 to kWh/m2. The InchPound unit conversion input option applies to all annual, monthly, timebins and economic reports that appear in the JSON file. The InchPoundExceptElectricity unit conversion input option makes conversions similar to what InchPound option does, except for using kWh (energy) or W/kW (rate) for electricity related outputs. This also applies to some lighting and plug load design levels that appears in IP version of Codes and Standards (usually given in W or W/ft$^2$).

\paragraph{Field: Format Numeric Values for Tabular Data}\label{field-format-numeric-values-for-tabular-data-json}

This field determines if any rounding will be performed for the values in the tabular reports when the Option field is set to TimeSeriesAndTabular. If No, all digits are shown after the decimal point without any rounding (23.238769213). If Yes, values are rounded for readability (23.24). The number of digets will vary depending on the type of value. This formatting is only applied to the tabular reports in the JSON file. This field has no effect on the time series data.
...
\paragraph{Field: Format Numeric Values for Tabular Data}\label{field-format-numeric-values-for-tabular-data}

This field determines if any rounding will be performed for the values in the tabular reports when the Option field is set to SimpleAndTabular. If No, all digits are shown after the decimal point without any rounding (23.238769213). If Yes, values are rounded for readability (23.24). The number of digets will vary depending on the type of value. This formatting is only applied to the tabular reports in the SQL file. This field has no effect on the time series data and predefined database tables generated by the Simple option (see the list in Table \ref{table:tabular_unit_conversion_list}). This field is optional and, if not specified, defaults to Yes.
