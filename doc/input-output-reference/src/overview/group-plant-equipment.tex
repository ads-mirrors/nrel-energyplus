...
simulation.
\paragraph{Field: Operating Mode Control Method} There are three options,
...
\end{itemize}
\paragraph{Field: Operating Mode Control Schedule Name} This field is used if
...
equation ($V = Q / (\rho \cdot Cp \cdot \Delta T)$) to solve for the volumetric flow rate.
...
\paragraph{Field: Maximum Outdoor Air Temperature in Heating Mode} This field is
...
\paragraph{Field: Minimum Leaving Water Temperature Curve Name in Heating Mode}
This optional alpha field specifies the name of a univariate curve or table that defines 
the minimum leaving water temperature. When the air-water temperature combination is outside of 
the operation boundary, the heat pump will not operate.
\paragraph{Field: Maximum Leaving Water Temperature Curve Name in Heating Mode}
This optional alpha field specifies the name of a univariate curve or table that defines 
the maximum leaving water temperature. When the air-water temperature combination is outside of 
the operation boundary, the heat pump will not operate.
...
This value could be either user-specified or autosized based on peak load and design
temperature difference.
\paragraph{Field: Minimum Outdoor Air Temperature in Cooling Mode} This field is
...
\paragraph{Field: Minimum Leaving Water Temperature Curve Name in Cooling Mode}
This optional alpha field specifies the name of a univariate curve or table that defines 
the minimum leaving water temperature. When the air-water temperature combination is outside of 
the operation boundary, the heat pump will not operate.
\paragraph{Field: Maximum Leaving Water Temperature Curve Name in Cooling Mode}
This optional alpha field specifies the name of a univariate curve or table that defines 
the maximum leaving water temperature. When the air-water temperature combination is outside of 
the operation boundary, the heat pump will not operate.
...
\paragraph{Field: Heat Pump Multiplier} This field specifies the number of heat 
pumps being modeled. The value must be an integer greater than or equal to 1. 
The default value is 1.0, which means only one heat pump is used. This input 
field is used to model modular heat pumps where multiple heat pump units are 
connected in parallel to provide larger heating/cooling capacity.
...
\item Heat Pump Speed Level in Heating Mode {[}{]}
\item Heat Pump Speed Ratio in Heating Mode {[}{]}
...
\item Heat Pump Speed Level in Cooling Mode {[}{]}
\item Heat Pump Speed Ratio in Cooling Mode {[}{]}
...
\end{itemize}
\paragraph{Heat Pump Part Load Ratio in Heating Mode {[}{]}} The part load ratio
...
\paragraph{Heat Pump Load Side Outlet Temperature in Heating Mode {[}C{]}} The
...
\paragraph{Heat Pump Defrost Electricity Energy {[}J{]}} The electricity energy
...
This output represents the number of heat pump units that are currently operating
...
It can range from 0 up to the maximum number of available heat pumps. For single-unit heat pumps,
this value will be either 0 (off) or 1 (on).
\paragraph{Heat Pump Speed Level in Heating Mode {[}{]}} The speed level of the
...
this value is a continuous non-negative value. When the speed level is 0,
the heat pump used for cooling is turned off.
\paragraph{Heat Pump Speed Ratio in Heating Mode {[}{]}} For a multi-speed heat pump, the speed ratio
indicates how far the current operating speed is between the two adjacent discrete speed levels. 
When the load is between the capacity of speed level n and speed level n-1, the speed ratio is calculated as: 
speed ratio = (load - capacity at speed n-1) / (capacity at speed n - capacity at speed n-1).
For a variable-speed heat pump, the speed ratio equals the part load ratio.
When the heat pump is cycling, the speed ratio is 0, and the proportion of time that it is on is reported in cycling ratio. 
...
\paragraph{Heat Pump Outlet Air Temperature in Heating Mode {[}C{]}}
...
It can range from 0 up to the maximum number of available heat pumps. For single-unit heat pumps,
this value will be either 0 (off) or 1 (on).
\paragraph{Heat Pump Speed Level in Cooling Mode {[}{]}} The speed level of the
heat pump in cooling mode. It is the current speed level of the compressor. For
fixed-speed heat pumps, this value is integers. For variable-speed heat pumps,
this value is a continuous non-negative value. When the speed level is 0,
the heat pump used for cooling is turned off.
\paragraph{Heat Pump Speed Ratio in Cooling Mode {[}{]}} For a multi-speed heat pump, the speed ratio
indicates how far the current operating speed is between the two adjacent discrete speed levels. 
When the load is between the capacity of speed level n and speed level n-1, the speed ratio is calculated as: 
speed ratio = (load - capacity at speed n-1) / (capacity at speed n - capacity at speed n-1).
For a variable-speed heat pump, the speed ratio equals the part load ratio.
...
The air flow rate of the heat pump in cooling mode in m3/s.
\paragraph{Heat Pump Inlet Air Temperature in Cooling Mode {[}C{]}}
The inlet air dry-bulb temperature of the heat pump in cooling mode.
\paragraph{Heat Pump Outlet Air Temperature in Cooling Mode {[}C{]}}
The outlet air dry-bulb temperature of the heat pump in cooling mode.
\paragraph{Heat Pump Capacity Temperature Modifier in Cooling Mode {[}{]}}
The capacity temperature modifier function value of the heat pump in cooling
mode. When the load is between the capacity of two speed levels, if the heat
pump is fixed-speed, this value is the capacity temperature modifier curve value
at the higher speed level; if the heat pump is variable-speed, this value is the
interpolated capacity temperature modifier between those two speed levels.
