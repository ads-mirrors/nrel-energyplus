...
This required numeric field contains the nominal capacity of the ice storage in GJ (Giga is 10\(^{9}\)). This field is autosizable when Coil:Cooling:Water, Coil:Cooling:Water:DetailedGeometry, AirTerminal:SingleDuct:ConstantVolume:CooledBeam or AirTerminal:SingleDuct:ConstantVolume:FourPipeBeam objects are used within the plant. Other plant equipment have not yet been added to the autosizing capability. If other plant equipment types are connected to the thermal energy storage plant loops and the TES capacity is insufficent to meet the load the work around for capacity sizing is to enter a value here instead of using autosize.
