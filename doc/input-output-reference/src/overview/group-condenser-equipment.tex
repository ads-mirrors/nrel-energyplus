...
\begin{enumerate}
    ...
    \item Request borehole field design and sizing by providing a single \hyperref[groundheatexchangerverticalsingle]{GroundHeatExchanger:Vertical:Single} object and also a \hyperref[groundheatexchangerverticalsizing]{GroundHeatExchanger:Vertical:Sizing} object.
\end{enumerate}
...
If the GHE:ResponseFactors object is not provided, this input will be relevant to the calculation method used to generate the g-function values.
There are three options for this input: ``UHFcalc'',  ``UBHWTcalc'', or ``FullDesign''.
The default is UHFcalc, where a g-function is computed with the uniform heat flux boundary condition.
The UBHWTcalc option computes the g-function with a uniform borehole wall temperature boundary condition.
The g-function computed with a uniform borehole wall temperature boundary condition is expected to be a better approximation of the borehole physics and is also expected to be faster to compute.
The final option of FullDesign allows a borehole field to be designed and sized based on loads and design parameters.
For a designed borehole field, the input file should include a GHE:Vertical:Sizing object matching the next field in this object, a full annual weather file sizing period to allow a year of loads to be generated, and finally hvac sizing enabled in the simulation control object.

\paragraph{Field: GHE:Vertical:Sizing object name}

For borehole field design and sizing calculations, this should be the name of an object specifying design parameters for the borehole field.  See the full documentation for \hyperref[groundheatexchangerverticalsizing]{GroundHeatExchanger:Vertical:Sizing}.
...
The following are example inputs corresponding to the input variations allowed which were described previously. The first example uses the \hyperref[groundheatexchangerverticalarray]{GroundHeatExchanger:Vertical:Array} object. The g-functions will be generated automatically by EnergyPlus.
...
The second example uses the \hyperref[groundheatexchangerverticalsingle]{GroundHeatExchanger:Vertical:Single} object. The g-functions will be generated automatically by EnergyPlus.
...
\subsection{GroundHeatExchanger:Vertical:Sizing}\label{groundheatexchangerverticalsizing}

The ground loop heat exchanger can be designed and sized using the packaged GHEDesigner library.
Currently the design is limited to rectangular borehole fields, with future enhancements expected to allow more flexible field design.
To achieve the design, several design parameters are required, and are supplied in this object.

\subsubsection{Inputs}

\paragraph{Field: Name}

This alpha field contains the identifying name for the ground heat exchanger sizing object.
This name will be referenced in a GroundHeatExchanger:System object.

\paragraph{Field: Borehole Field Geometry Type}

This alpha field contains the borehole field geometry.
As of now, the only possible input here is Rectangle, which will try to design the best borehole field in a rectangular space.
GHEDesigner supports other field geometry, so it is expected that future versions of EnergyPlus will allow more diverse field types.

\paragraph{Field: Available Borehole Field Length}

This is the available borehole field length, in meters, as looking down from above.

\paragraph{Field: Available Borehole Field Width}

This is the available borehole field width, in meters, as looking down from above.

\paragraph{Field: Maximum Number of Boreholes}

This is the maximum number of boreholes available for the design.

\paragraph{Field: Minimum Borehole Spacing}

This is the minimum borehole-to-borehole spacing, in meters.

\paragraph{Field: Maximum Borehole Spacing}

This is the maximum borehole-to-borehole spacing, in meters.

\paragraph{Field: Minimum Borehole Vertical Length}

This is the minimum vertical height, or total borehole ``length'', to be used when designing the borehole field.

\paragraph{Field: Maximum Borehole Vertical Length}

This is the maximum vertical height, or total borehole ``length'', to be used when designing the borehole field.

\paragraph{Field: Minimum Exiting Fluid Temperature for Sizing}

This is the minimum allowable exiting fluid temperature for the borehole heat exchanger when designing the borehole field.

\paragraph{Field: Maximum Exiting Fluid Temperature for Sizing}

This is the maximum allowable exiting fluid temperature for the borehole heat exchanger when designing the borehole field.

\subsubsection{Example Inputs}

This is an example input object for the sizing parameters:

\begin{lstlisting}
  GroundHeatExchanger:Vertical:Sizing,
    GHE-1-Sizing,            !- Name
    Rectangle,               !- Borehole Field Geometry Type
    100.0,                   !- Available Borehole Field Length
    100.0,                   !- Available Borehole Field Width
    200.0,                   !- Maximum Number of Boreholes
    3.0,                     !- Minimum Borehole Spacing
    10.0,                    !- Maximum Borehole Spacing
    60.0,                    !- Minimum Borehole Vertical Length
    135.0,                   !- Maximum Borehole Vertical Length
    5.0,                     !- Minimum Exiting Fluid Temperature for Sizing
    35.0;                    !- Maximum Exiting Fluid Temperature for Sizing
\end{lstlisting}

\subsubsection{Outputs}

This object does not have any specific output variables.
However, by using the full design approach, there may be additional output files or sizing outputs available in the EIO.
